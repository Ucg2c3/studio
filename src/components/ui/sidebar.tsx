...
>(({ className, onClick, children, asChild, ...restButtonProps }, ref) => {
  ...
  return (
    <Button
      ...
      className={cn("h-7 w-7", className)} // Merge className
      onClick={(event) => {
        onClick?.(event); // Call user-provided onClick if any
        toggleSidebar();
      }}
      asChild={asChild} // Pass asChild to the underlying Button
      {...restButtonProps} // Pass remaining ButtonProps
    >
      {/* Conditionally render children or default content based on asChild */}
      {asChild ? children : (
        <>
          <PanelLeft />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col overflow-auto group-data-[collapsible=icon]:overflow-hidden", // Removed gap-2
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn("relative flex w-full min-w-0 flex-col p-2 py-1", className)} // Adjusted padding
      {...props}
    />
  )
})
